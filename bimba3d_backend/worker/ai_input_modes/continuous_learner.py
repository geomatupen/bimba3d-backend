...
def _selector_dir(project_dir: Path) -> Path:
    return project_dir / "models" / "input_mode_selector_continuous"
...
def _default_mode_state() -> dict[str, Any]:
    return {
        "means": {k: 1.0 for k in MULT_KEYS},
        "runs": 0,
        "reward_mean": 0.0,
        "last": {},
    }
...
def _mode_entry(model: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    entry = modes.setdefault(mode, _default_mode_state())
    means = entry.get("means")
    if not isinstance(means, dict):
        entry["means"] = {k: 1.0 for k in MULT_KEYS}
    else:
        for k in MULT_KEYS:
            if k not in means:
                means[k] = 1.0
...
def _build_updates(params: dict[str, Any], multipliers: dict[str, float]) -> dict[str, Any]:
    ...
    return {
        "preset_name": "continuous_bandit_linear",
...
def select_continuous(
    *,
    ...
    exploration_std = max(0.03, min(0.18, 0.18 / math.sqrt(float(runs + 1))))
    sampled: dict[str, float] = {}
    for key in MULT_KEYS:
        lo, hi = SAFE_BOUNDS[key]
        mu = float(means.get(key, 1.0) or 1.0)
        sampled_value = random.gauss(mu, exploration_std)
        sampled[key] = float(clamp_float(sampled_value, lo, hi))

    updates = _build_updates(params, sampled)

    return {
        "selected_preset": "continuous_bandit_linear",
        "yhat_scores": {k: float(v) for k, v in sampled.items()},
        "updates": updates,
        "exploration_std": exploration_std,
    }


def update_from_run_continuous(
    *,
    project_dir: Path,
    mode: str,
    selected_preset: str,
...
def record_run_penalty_continuous(
    *,
    project_dir: Path,
    mode: str,
    selected_preset: str,
