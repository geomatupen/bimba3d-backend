...
try:
    import tyro
except Exception:  # pragma: no cover - optional for non-CLI runtime
    tyro = None

try:
    import viser
except Exception:  # pragma: no cover - optional for headless runtime
    viser = None

import yaml
try:
    from gsplat.color_correct import color_correct_affine, color_correct_quadratic
except Exception:  # pragma: no cover - older gsplat builds may not expose this module
    def color_correct_affine(pred: Tensor, target: Tensor) -> Tensor:
        return pred

    def color_correct_quadratic(pred: Tensor, target: Tensor) -> Tensor:
        return pred
...
except Exception:  # pragma: no cover - fallback when fused kernel pkg is unavailable
    from torchmetrics.functional.image import structural_similarity_index_measure

    def fused_ssim(pred: Tensor, target: Tensor, padding: str = "valid") -> Tensor:
        del padding
        # Call site already passes NCHW tensors.
...
except Exception:  # pragma: no cover - viewer stack is optional in backend runtime
    GsplatViewer = None
    GsplatRenderTabState = object
    CameraState = object
    RenderTabState = object

    def apply_float_colormap(*args, **kwargs):
        raise RuntimeError("Viewer dependencies are unavailable")
...
class Runner:
    ...
    def __init__(
        ...
        elif not self.cfg.disable_viewer:
            print("Viewer dependencies unavailable, forcing headless mode.")
            self.cfg.disable_viewer = True
    ...
    def train(self):
        ...
        default_workers = 0 if os.environ.get("BIMBA3D_DOCKER_WORKER") == "1" else 4
        num_workers = int(os.environ.get("GSPLAT_NUM_WORKERS", str(default_workers)))
        use_worker_threads = num_workers > 0
        ...
        trainloader = torch.utils.data.DataLoader(
            ...
            persistent_workers=use_worker_threads,
            pin_memory=use_worker_threads,
        )
        ...
        for step in pbar:
            if callable(stop_checker) and stop_checker():
                stopped_step = step + 1
                print(f"Stop requested. Halting training at step {step + 1}.")
                break
    ...
    def eval(self, step: int, stage: str = "val"):
        ...
        val_num_workers = 0 if os.environ.get("BIMBA3D_DOCKER_WORKER") == "1" else 1
        valloader = torch.utils.data.DataLoader(
            self.valset, batch_size=1, shuffle=False, num_workers=val_num_workers
        )
        ellipse_time = 0
